...
结构体字段的可见性
结构体中字段大写开头表示可公开访问，小写表示私有（仅在定义当前结构体的包中可访问）。私有不能被json包访问
结构体与JSON序列化
...
注意事项： 为结构体编写Tag时，必须严格遵守键值对的规则。例如：不要在key和value之间添加空格。
...
func jsonDemo() {
	u2 := User{
		name:"老王",
		gender:"男",
		Address:Address{"哈尔滨","黑龙江","20201111"},
		Email:Email{"黑龙江","20201111"},
	}
	fmt.Printf("u2=%#v\n",u2)
	//JSON序列化：结构体-->JSON格式的字符串
	data, error := json.Marshal(u2)
	if error != nil {
